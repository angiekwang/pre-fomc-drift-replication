...
portfolio_data  <- read.csv(here("Desktop", "fomc-drift", "data", "raw", "Portfolios_Formed_on_ME_daily.csv"))
famafrench_data <- read.csv(here("Desktop", "fomc-drift", "data", "raw", "F-F_Research_Data_Factors_daily.CSV"))
...
# Clean rows to only show dates between 1994-09-01 and 2011-03-30

decile_returns$Date <- as.Date( # Convert to date format
  as.character(decile_returns$Date),
  format = "%Y%m%d"
  )

decile_returns <- decile_returns %>% filter( # Filter to sample period
  Date >= "1994-09-01" & Date <="2011-03-30"
  )

decile_returns <- decile_returns %>% mutate( # Create year variable
  year = year(Date)
  )

# Add 'fomc' dummy variable: 1 if fomc announcement date, 0 otherwise

fomc_dates <- c("19940927", "19941115", "19941220", # List of FOMC dates obtained from Fed website
...
decile_returns <- decile_returns %>% mutate( # Create dummy variable for FOMC announcement date
  fomc = ifelse(
    Date %in% fomc_dates, 1, 0
    )
  )
...
saveRDS(decile_returns, "/Users/angiewang/Desktop/fomc-drift/data/processed/decile_returns_clean.rds")

# Save decile_returns dataframe as .csv file (without row numbers)
write.csv(decile_returns,
          "/Users/angiewang/Desktop/fomc-drift/data/processed/decilereturns.csv",
          row.names = FALSE)

# Save as .dta file
haven::write_dta(
  decile_returns,
  "/Users/angiewang/Desktop/fomc-drift/data/processed/decilereturns.dta"
)
